...
function getPropertySummary(property, knownTypes, autoLink) {
    ...
    if (type === 'array') {
        ...
        // if the items type is a schema
        if (defined(property.items) && defined(property.items.typeName))
        {
            type = property.items.typeName;
            formattedType = `${style.linkType(style.typeValue(type), type, autoLink)  } `;

            type += arrayInfo;
            formattedType = style.typeValue(type)
        }
        else if (defined(property.items) && defined(property.items.type)) {
